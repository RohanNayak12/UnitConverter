...
@Composable
fun MassConverter(nvCntrl: NavHostController) {
    Scaffold(topBar = { UpperPanel(nvCntrl) }) {it
        Column(modifier = Modifier
            .fillMaxSize()
            .background(Color.White)
            .padding(top = 140.dp)) {
            Text(text = "Mass Unit Conversions", color = Color.Black,fontSize=34.sp, modifier = Modifier.padding(start = 13.dp))
            InputTaker(List = MassList)
            MassCal()
        }
    }
}
...
fun MassCal(){
    ...
    Row(
        ...
        Button(onClick = { ResultAnswer= KgToConverter(MassConverterToKg(unitValue,
