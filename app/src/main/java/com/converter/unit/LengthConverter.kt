...
@Composable
fun LengthConverter(nvCntrl: NavHostController) {
    Scaffold(topBar = { UpperPanel(nvCntrl) }) {it
        Column(modifier = Modifier
            ...
            Text(text = "Length Unit Conversions", color = Color.Black,fontSize=34.sp, modifier = Modifier.padding(start = 13.dp))
            InputTaker(List = LenghtList)
            LengthCal()
...
fun LengthCal(){
    ...
    Row(
        ...
        Button(onClick = { ResultAnswer= MeterToConverter(lengthConverterToMeter(unitValue,
    ...
    Row(modifier = Modifier
        ...
        Text(text = "$textResult  $secondUnit", fontSize = 35.sp, fontWeight = FontWeight.Bold )
    }
}